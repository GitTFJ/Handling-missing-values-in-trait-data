SummarisedImputationErrors_MAE = SummarisedImputationErrors %>%
  ...
  summarise(MedianAE = mean(MedianAE, na.rm = T))
...
NoBias = ggplot() + 
  geom_smooth(data = Ribbon[which(Ribbon$FunctionReName == "Random"),],
              aes(y = MedianAE, x = Missing, fill = ImputationApproach, linetype = ImputationApproach), alpha = 0.3, method = "lm", colour = "black") +
  ...
  labs(title = "No bias", x = " ", y = "Median absolute error")
...
ggsave("../Results/SummaryPlots/MedianAEVsMissingVsBias.png" ,plot = last_plot(), width = 90, height = 45, units = "cm")
