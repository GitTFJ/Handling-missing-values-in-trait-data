Temp = CIPredCombinedRegressionErrors_Positive
Temp = subset(Temp, (ImputationApproach == "Rphylopars" | ImputationApproach == "Mice: mean matching + phylogeny" | ImputationApproach == "Mice: regression + phylogeny" | ImputationApproach == "Mice: random forest + phylogeny" | ImputationApproach == "Complete-case" | ImputationApproach == "BHPMF + phylogeny") & Trend == "PositiveTrend")
...
Temp = subset(Temp, (ImputationApproach == "Rphylopars" | ImputationApproach == "Mice: mean matching + phylogeny" | ImputationApproach == "Mice: regression + phylogeny" | ImputationApproach == "Mice: random forest + phylogeny" | ImputationApproach == "Complete-case" | ImputationApproach == "BHPMF + phylogeny") & Trend == "NoTrend")
...
Plot = annotate_figure(Plot,
                       left = text_grob(paste("Slope error " , "\u00B1"," 95% confidence intervals", sep = ""), color = "black", rot = 90, size = 35))
ggsave("../Results/SummaryPlots/RegressionErrorVsMissingVsBiasRefined.pdf" ,plot = last_plot(), width = 70, height = 50, units = "cm")
